...
export default {
  ...
  "POST /mock/theme/remove": async (req: any, res: any) => {
    const { key } = req.body;
    themeList = themeList.filter((item) => item.key !== key);
  ...
  /** 更新主题详情 */
  "POST /mock/theme/theme/update": async (req: any, res: any) => {
    const { key, type, label, desc, name, value, id } = req.body;
    const index = themeItemList.findIndex((item) => item.id === id);
    themeItemList.splice(index, 1, { type, label, desc, name, value, id });
    changeHash[key] = new Date().getTime();
    res.json({ code: "0", message: "成功！" });
  },
};
